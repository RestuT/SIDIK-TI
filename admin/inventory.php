...
if (!empty($search_query)) {
    $search_where = " WHERE (i.item_name LIKE '%$search_query%' OR i.category LIKE '%$search_query%') ";
}

// 1. Query JOIN untuk menampilkan data stok + detail dari master template (dengan filter)
$query = "SELECT 
            i.*, 
            t.specification, 
        ...
        <div class="p-8 space-y-8">
            ...
            <section class="grid grid-cols-1 md:grid-cols-4 gap-6">
                ...
                <div class="bg-white p-6 rounded-3xl shadow-sm border border-outline-variant/5 flex flex-col justify-between group hover:shadow-md transition-all">
                    ...
                    <div>
                        <?php 
                            $stat_low = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM inventory WHERE stock <= min_stock"))['total'];
                        ?>
                ...
                <div class="bg-surface-container-high p-6 rounded-3xl flex flex-col justify-between group hover:bg-white hover:shadow-md transition-all">
                    ...
                    <div>
                        <?php 
                            $stat_cat = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(DISTINCT category) as total FROM inventory"))['total'];
                        ?>
                            ...
                            <tr class="group hover:bg-surface-container-lowest transition-all">
                                ...
                                <td class="px-8 py-6 text-center">
                                    <div class="flex flex-col items-center">
                                        <span class="text-2xl font-black <?php echo $row['stock'] <= $row['min_stock'] ? 'text-error animate-pulse' : 'text-on-surface'; ?>"><?php echo $row['stock']; ?></span>
                                        <span class="text-[10px] font-black text-outline/60 uppercase tracking-widest"><?php echo strtoupper($row['satuan']); ?></span>
                                    </div>
                                </td>
                                <td class="px-8 py-6 text-right">
