...
$service_query = mysqli_query($conn, "SELECT type, COUNT(*) as total FROM submissions GROUP BY type");
$service_labels = [];
$service_data = [];
...
$asset_query = mysqli_query($conn, "SELECT category, COUNT(*) as total FROM asset_assignments GROUP BY category");
$asset_labels = [];
$asset_data = [];
while ($row = mysqli_fetch_assoc($asset_query)) {
    $asset_labels[] = $row['category'];
    $asset_data[] = (int)$row['total'];
}
...
for ($i = 5; $i >= 0; $i--) {
    ...
    $month_start = date('Y-m-d', strtotime("first day of -$i months"));
    $month_end = date('Y-m-d', strtotime("last day of -$i months"));
    
    $count = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM submissions WHERE DATE(created_at) BETWEEN '$month_start' AND '$month_end'"))['total'];
    
    $trend_labels[] = $month_label;
    $trend_data[] = (int)$count;
}

// --- 4. RINGKASAN STATISTIK ---
$stat_total_assets = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM asset_assignments"))['total'];
$stat_avg_completion = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM submissions WHERE status = 'Selesai'"))['total'];
$stat_pending = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM submissions WHERE status = 'Menunggu'"))['total'];
?>
