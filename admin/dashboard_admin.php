...
// Ambil Statistik
$stat_total = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM submissions"))['total'];
$stat_pending = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM submissions WHERE status = 'Menunggu'"))['total'];
$stat_process = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM submissions WHERE status = 'Proses'"))['total'];
$stat_done = mysqli_fetch_assoc(mysqli_query($conn, "SELECT COUNT(*) as total FROM submissions WHERE status = 'Selesai'"))['total'];
...
if (!empty($search_q)) {
    $where_clauses[] = "(s.ticket_number LIKE '%$search_q%' OR u.full_name LIKE '%$search_q%' OR s.title LIKE '%$search_q%')";
}

$where_sql = implode(" AND ", $where_clauses);

// Ambil Semua Pengajuan (dengan Filter)
$query = "SELECT s.*, u.full_name as user_name, u.department 
          FROM submissions s 
                                    ...
                                    <tr class="group hover:bg-surface-variant/10 transition-all">
                                        ...
                                        <td class="px-8 py-6">
                                            <div class="flex flex-col">
                                                <span class="text-sm font-bold text-on-surface"><?php echo htmlspecialchars($row['user_name']); ?></span>
