    ...
    <main class="max-w-5xl mx-auto px-6 py-12 space-y-10">
        ...
        <?php else: ?>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <?php foreach ($tasks as $task): ?>
                    <div class="glass-card p-8 rounded-[2rem] shadow-sm hover:shadow-xl transition-all border border-outline/5 relative overflow-hidden group">
                        <?php if ($task['status'] === 'Reported' || $task['status'] === 'Finalized'): ?>
                            <div class="absolute top-0 right-0 px-6 py-2 bg-emerald-500 text-white text-[10px] font-black uppercase tracking-widest rounded-bl-3xl">Selesai</div>
                        <?php endif; ?>
                        ...
                        <div class="space-y-4">
                            ...
                            <?php if ($task['status'] === 'Pending'): ?>
                                <button onclick="bukaModalSensus('<?php echo $task['id']; ?>', '<?php echo htmlspecialchars($task['item_name']); ?>')" class="w-full py-4 bg-primary text-white rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-lg shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all">Mulai Laporan</button>
                            <?php else: ?>
                                <div class="p-4 bg-surface-container-low rounded-2xl border border-outline/5">
                                    ...
                                    <div class="flex items-center gap-2">
                                        <div class="h-1.5 flex-1 bg-slate-200 rounded-full overflow-hidden"><div class="h-full bg-emerald-500" style="width: <?php echo $task['report_pct']; ?>%"></div></div>
                ...
                <?php endforeach; ?>
                <?php if (empty($tasks)): ?>
                    <div class="col-span-full py-20 text-center obsidian-panel rounded-[2.5rem]"><span class="material-symbols-outlined text-5xl opacity-10">assignment_turned_in</span><p class="text-on-surface-variant font-medium mt-4">Anda tidak memiliki aset yang perlu disensus pada periode ini.</p></div>
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </main>
    ...
    <div id="modalSensus" class="fixed inset-0 bg-slate-900/60 hidden items-center justify-center z-[100] p-6 backdrop-blur-sm transition-all">
        <div class="bg-surface rounded-[2.5rem] max-w-md w-full p-10 shadow-2xl relative border border-white/10">
            ...
            <div class="mb-8 flex flex-col items-center text-center">
                <div class="w-16 h-16 bg-primary/10 text-primary flex items-center justify-center rounded-3xl mb-4"><span class="material-symbols-outlined text-3xl">fact_check</span></div>
            ...
            <form action="../config/proses_sensus.php" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <div class="relative"><input type="number" name="condition_pct" min="0" max="100" value="100" required class="w-full text-center py-6 bg-surface-container-low border-0 rounded-3xl focus:ring-4 focus:ring-primary/10 outline-none font-black text-4xl text-on-surface"></div>
                </div>
                <div>
                    ...
                    <textarea name="notes" placeholder="Tuliskan kendala jika ada..." rows="3" class="w-full p-5 bg-surface-container-low border-0 rounded-3xl focus:ring-4 focus:ring-primary/10 outline-none font-medium text-sm text-on-surface"></textarea>
                </div>
                <button type="submit" class="w-full bg-primary text-white font-black py-5 rounded-3xl shadow-xl shadow-primary/30 hover:shadow-primary/50 hover:scale-[1.02] active:scale-95 transition-all uppercase tracking-[0.2em] text-xs">Kirim Laporan Sensus</button>
            </form>
        </div>
    </div>
