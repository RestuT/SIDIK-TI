...
if (!$db && $conn) {
    ...
    $res = mysqli_query($conn, "SELECT s.*, u.full_name, u.jabatan, u.department FROM submissions s JOIN users u ON s.user_id = u.id WHERE s.id = '$id_e' AND s.user_id = '$uid_e' AND s.type = 'Pengadaan' LIMIT 1");
...
<head>
    <?php 
        $pageTitle = 'SIDIK-TI | Cetak Tiket Pengadaan';
        $base_url = '../';
...
<body class="bg-surface p-6 md:p-12 font-body text-slate-900 antialiased transition-colors duration-300">
    <div class="max-w-3xl mx-auto bg-white p-12 rounded-[3rem] shadow-2xl border border-outline-variant/10 relative overflow-hidden">
        ...
        <div class="flex flex-col items-center border-b-2 border-dashed border-slate-100 pb-10 mb-10 relative z-10">
            <div class="w-16 h-16 bg-orange-50 text-orange-600 rounded-2xl flex items-center justify-center mb-6"><span class="material-symbols-outlined text-3xl fill-1">receipt_long</span></div>
            ...
            <div class="mt-4 px-4 py-1.5 bg-slate-50 border border-slate-100 rounded-full"><p class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Procurement Control: <span class="text-orange-600">#<?php echo htmlspecialchars($data['ticket_number'] ?? ''); ?></span></p></div>
        </div>
        ...
        <div class="space-y-8 mb-12">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div class="flex flex-col gap-2">
                    <div class="flex items-center gap-2"><span class="material-symbols-outlined text-orange-600 text-xl">inventory_2</span><p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Item & Spesifikasi</p></div>
                    <p class="font-headline font-black text-xl text-slate-800"><?php echo htmlspecialchars($data['title'] ?? ''); ?></p>
                </div>
                <div class="flex flex-col md:items-end justify-center">
                    <div class="flex items-center gap-2 mb-1.5"><span class="material-symbols-outlined text-orange-600 text-xl">bolt</span><p class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Level Urgensi</p></div>
                    <span class="px-5 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border <?php echo ($data['urgency'] ?? '') == 'Penting' ? 'bg-error/5 text-error border-error/10' : 'bg-primary/5 text-primary border-primary/10'; ?>"><?php echo htmlspecialchars($data['urgency'] ?? ''); ?></span>
                </div>
            </div>
        ...
        <div class="border-t-2 border-dashed border-slate-100 pt-10">
            ...
            <div class="flex flex-col md:flex-row gap-4 no-print">
                <button onclick="window.print()" class="flex-[2] py-5 bg-slate-900 text-white font-headline font-black rounded-2xl shadow-xl hover:bg-orange-600 transition-all uppercase tracking-[0.2em] text-[10px] flex items-center justify-center gap-3"><span class="material-symbols-outlined text-xl">print</span>Print Official Document</button>
