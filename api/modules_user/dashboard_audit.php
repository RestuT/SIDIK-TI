...
if ($db) {
    try {
        $submissions_docs = $db->collection('submissions')->where('user_id', '=', $user_id)->orderBy('created_at', 'DESC')->documents();
...
if (!$db && $conn) {
    ...
    $res_pic = mysqli_query($conn, "SELECT id, full_name FROM users WHERE role = 'admin'");
    while ($row = mysqli_fetch_assoc($res_pic)) { $pic_map[$row['id']] = $row['full_name']; }
    $res_subs = mysqli_query($conn, "SELECT * FROM submissions WHERE user_id = '$uid_e' ORDER BY created_at DESC");
    while ($row = mysqli_fetch_assoc($res_subs)) {
        $row['pic_name'] = $pic_map[$row['pic_id'] ?? ''] ?? null;
        $submission_list[] = $row;
    }
}
