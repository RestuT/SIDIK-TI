...
<body class="bg-background text-on-surface min-h-screen selection:bg-primary/20 pb-24 md:pb-0 transition-colors duration-300">
    ...
    <!-- Hero Section -->
    <div class="relative pt-20 pb-16 px-6 lg:px-8 overflow-hidden bg-white border-b border-indigo-50">
        <!-- Dekorasi Background -->
        <div class="absolute inset-y-0 right-1/2 -z-10 mr-16 w-[200%] origin-bottom-left skew-x-[-30deg] bg-white shadow-xl shadow-indigo-600/10 ring-1 ring-indigo-50 sm:mr-28 lg:mr-0 xl:mr-16 xl:origin-center"></div>
        <div class="absolute top-0 right-0 p-32 opacity-5 mix-blend-multiply pointer-events-none -z-10">
            <span class="material-symbols-outlined text-[300px]">lightbulb</span>
        </div>

        <div class="max-w-7xl mx-auto text-center space-y-6">
            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-indigo-50 border border-indigo-100 text-indigo-700 font-bold text-xs uppercase tracking-widest mb-2 shadow-sm">
                <span class="material-symbols-outlined text-[14px]">library_books</span>
                Pusat Bantuan IT
            </div>
            <h1 class="text-4xl md:text-6xl font-black font-headline tracking-tight text-slate-900 italic">
                Knowledge <span class="text-primary italic">Base</span>
            </h1>
            <p class="mt-4 text-base md:text-lg leading-8 text-slate-500 max-w-2xl mx-auto font-medium">
            ...
            <div class="mt-8 max-w-xl mx-auto relative group">
                ...
                <input type="text" placeholder="Cari masalah Anda (misal: Printer rusak...)" class="block w-full pl-11 pr-4 py-4 md:py-5 border-slate-200 rounded-2xl md:rounded-3xl shadow-lg shadow-slate-200/50 focus:ring-primary focus:border-primary sm:text-base font-medium transition-all group-hover:shadow-xl">
            </div>
        </div>
    </div>
    ...
    <main class="max-w-4xl mx-auto px-6 md:px-10 py-16 space-y-12">
        ...
        <section class="space-y-6">
            <div class="flex items-center gap-3 border-b border-slate-200 pb-4">
                <div class="p-3 bg-gradient-to-br from-indigo-50 to-white rounded-xl shadow-sm border border-indigo-100 text-indigo-600">
                    <span class="material-symbols-outlined text-xl"><?php echo $faqSection['icon']; ?></span>
                </div>
                <h2 class="text-2xl font-black font-headline text-slate-800 tracking-tight"><?php echo $faqSection['category']; ?></h2>
            </div>

            <div class="space-y-4">
                <?php foreach ($faqSection['items'] as $item): ?>
                <div x-data="{ expanded: false }" class="bg-white border hover:border-primary/20 border-slate-200 rounded-2xl shadow-sm transition-all overflow-hidden group">
                    <button @click="expanded = ! expanded" class="w-full px-6 py-5 flex items-center justify-between text-left focus:outline-none">
                        <span class="text-[15px] font-bold text-slate-800 group-hover:text-primary transition-colors pr-6">
                            <?php echo htmlspecialchars($item['q']); ?>
                        </span>
                        <span class="material-symbols-outlined text-slate-400 transition-transform duration-300" :class="expanded ? 'rotate-180 text-primary' : ''">
                            expand_more
                        </span>
                    </button>
                    <!-- Expandable Content -->
                    <div x-show="expanded" x-collapse x-cloak class="px-6 pb-6 text-sm text-slate-600 font-medium leading-relaxed border-t border-slate-50 pt-4 bg-slate-50/50">
