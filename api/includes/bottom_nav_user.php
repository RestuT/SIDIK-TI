...
function get_mobile_nav_style($page, $current) {
    return $page == $current 
        ? 'flex flex-col items-center justify-center bg-indigo-50 text-indigo-700 rounded-2xl px-3 py-2 transition-all drop-shadow-sm' 
        : 'flex flex-col items-center justify-center text-slate-400 px-3 py-2 hover:bg-slate-50 rounded-2xl transition-all';
}
...
<nav class="md:hidden fixed bottom-0 left-0 w-full z-50 flex justify-around items-center px-4 py-3 pb-safe bg-white/90 dark:bg-slate-900/90 backdrop-blur-lg shadow-[0_-4px_20px_rgba(0,0,0,0.05)] border-t border-slate-100 rounded-t-3xl">
