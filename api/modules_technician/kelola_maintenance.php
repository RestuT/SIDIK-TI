...
if (!$db && $conn) {
    ...
    $res = mysqli_query($conn, "SELECT s.*, u.full_name as pemohon, u.department FROM submissions s LEFT JOIN users u ON s.user_id = u.id WHERE s.id = '$id_sql' AND s.type = 'Maintenance'");
...
<head>
    <meta charset="UTF-8">
    <title>Process Maintenance - #<?php echo htmlspecialchars($data['ticket_number']); ?></title>
...
<body class="bg-slate-50 font-sans antialiased min-h-screen">
    ...
    <main class="lg:ml-72 pt-14 lg:pt-0 min-h-screen tech-layout">
        <div class="max-w-4xl mx-auto p-6 md:p-10">
            <a href="dashboard_technician.php" class="inline-flex items-center text-sm font-bold text-gray-500 hover:text-indigo-600 mb-6 transition">
                <i class="fa-solid fa-arrow-left mr-2"></i> Kembali ke Antrean
            </a>
            <div class="bg-white rounded-[40px] shadow-sm border border-gray-100 overflow-hidden">
                <div class="bg-indigo-600 p-8 text-white flex justify-between items-center text-sm font-black uppercase tracking-widest opacity-80">
                    <div>
                        <p>Maintenance Assignment</p>
                ...
                <div class="p-10 grid grid-cols-1 md:grid-cols-2 gap-10">
                    <div class="space-y-6">
                        ...
                        <section>
                            ...
                            <div class="bg-indigo-50 p-5 rounded-2xl border border-indigo-100 italic text-sm text-gray-700">
                                <p class="text-xs font-bold text-indigo-700 underline mb-2 not-italic">Kerusakan:</p>
                    ...
                    <div class="bg-slate-50 p-8 rounded-[35px] border border-slate-100 h-fit">
                        <form action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>" method="POST" class="space-y-6">
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Status Pengerjaan</label>
                                <select name="status" class="w-full p-4 bg-white border border-gray-200 rounded-2xl font-black text-sm outline-none focus:ring-4 focus:ring-indigo-100 transition">
                                    <option value="Menunggu" <?php echo $data['status'] == 'Menunggu' ? 'selected' : ''; ?>>Belum Dikerjakan</option>
                                    <option value="Proses" <?php echo $data['status'] == 'Proses' ? 'selected' : ''; ?>>On Process</option>
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Catatan Hasil / Progress</label>
                                <textarea name="tech_note" rows="5" class="w-full p-4 bg-white border border-gray-200 rounded-2xl text-sm italic outline-none focus:ring-4 focus:ring-indigo-100 transition"><?php echo htmlspecialchars($data['admin_reasoning'] ?? ''); ?></textarea>
                            </div>
                            <button type="submit" name="update_tech" class="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-2xl shadow-lg transition transform active:scale-95 uppercase tracking-widest text-[10px]">
                                Update Report Tugas
                            </button>
                        </form>
                    </div>
